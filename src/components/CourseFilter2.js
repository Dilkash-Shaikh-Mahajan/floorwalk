...
function CourseFilterBottom() {
    ...
    const [dataList,setDataList]=useState(Datas.dataList.filter(function (data){
        return data.targetId==='Non-Purchase';
    }))

    useEffect(() => {
        const buttons = document.querySelector(".filter-btn-list2").children;
        ...
        for (let i = 0; i < buttons.length; i++) {
            buttons[i].addEventListener("click", function () {
                ...
                const target = this.getAttribute("data-target");
                 console.log(`${target}`);
                setDataList(Datas.dataList.filter(function (data){
                        ...
                        <Col md="12">
                            <div className="filter-btns2 text-center">
                                <ul className="filter-btn-list2 list-unstyled list inline">
                                    <li data-target="Non-Purchase" className="active list-inline-item">Non Purchase</li>
                                    <li data-target="Purchase" className="list-inline-item">Purchase</li>
                                    <li data-target="Web-Telphonic" className="list-inline-item">Web & Telphonic</li>
                                        ...
                                        <Col lg="4" md="6" key={i} data-id={data.targetId}>
                                            <div className="course-item2">
                                                <Link to={process.env.PUBLIC_URL + data.courseLink}>
                                                    <div className="course-image2" style={{backgroundImage: `url(${process.env.PUBLIC_URL}/assets/images/${data.imgUrl})`}}>
